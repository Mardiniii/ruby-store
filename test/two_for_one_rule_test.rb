...
class TwoForOneRuleTest < Test::Unit::TestCase
  def setup
    @rule = TwoForOneRule.new('VOUCHER')
  ...
  def test_when_checkout_has_an_even_number_of_products_to_apply_two_for_one_discount
  ...
  def test_when_checkout_has_an_odd_number_of_products_to_apply_two_for_one_discount
    ...
    assert_equal @voucher_one.price, @voucher.price, "Item price didn't change"
    assert_equal @voucher_two.price, 0, "Item price now is zero"
    assert_equal @voucher_three.price, @voucher.price, "Item price didn't change"
    assert_equal @mug_one.price, @mug_one.price, "Item price didn't change"
  end
end
